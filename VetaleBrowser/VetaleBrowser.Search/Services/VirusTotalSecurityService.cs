...
/// <summary>
/// VirusTotal Public API v3 client for URL verification
/// </summary>
public class VirusTotalSecurityService
{
    private const string BaseUrl = "https://www.virustotal.com/api/v3";

    // TODO: move to settings, currently a stub constant
    private const string ApiKey = "a0c5bf9c0c5547525f5fb9799ae52006ff3d2951f10a0730d5acbe8fd6f9e42c"; // If empty - VT is not used

    private readonly HttpClient _httpClient;

    // Simple cache for VT results
    ...
    {
        // If no key - simply don't use VT
        if (string.IsNullOrWhiteSpace(ApiKey))
            return null;

        if (DateTimeOffset.UtcNow < _rateLimitedUntil)
            return null; // temporarily not calling VT
    ...
    {
        // VT v3 requires encoding URL in base64 without trailing '='
    ...
    private static string NormalizeForCache(string url)
    {
        try
        {
            ...
            var uri = new Uri(url);
            // For cache - host + path without query, to avoid combinatorial explosion
